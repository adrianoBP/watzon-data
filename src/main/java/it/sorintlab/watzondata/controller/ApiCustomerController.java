...
public class ApiCustomerController {
	...
	/*
	 * @GetMapping(value = "/contacts") public List<APIContact> listAllcontacts() {
	 * ArrayList<Contact> temp = new ArrayList<>();
	 * contactRepository.findAll().forEach(temp::add); return temp.stream().map(c ->
	 * APIContact.fromBackend(c)).collect(Collectors.toList()); }
	 */
	...
	public List<String> listProductsByCustomer(@PathVariable("id") int id) {
		...
		return customerRepository.findOne(id).getProducts().stream()
				.map(c -> new String("/api/customers/" + temp.getId() + "/products/" + c.getId()))
	...
	public List<String> listContactByCustomer(@PathVariable("id") int id) {
		Customer temp = customerRepository.findOne(id);
		return customerRepository.findOne(id).getContacts().stream()
				.map(c -> new String("/api/customers/" + temp.getId() + "/contacts/" + c.getId()))
				.collect(Collectors.toList());
	}
